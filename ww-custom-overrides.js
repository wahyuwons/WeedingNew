(function(){
  ...
  function setupWeddingFrame(){
    ...
    host.innerHTML = `
      <section class="ww-frame-section" aria-labelledby="wwFrameTitle">
        ...
        <div class="ww-frame-preview">
          <canvas id="wwFrameCanvas" width="1080" height="1350" aria-label="Preview wedding frame"></canvas>
        </div>
    ...
    function render(){
      ...
      if(overlay){
        const margin = 55;
        ctx.drawImage(overlay,margin,35,w-(margin*2),h-70);
      }
      if(topOverlay){
      ...
      ctx.font = '400 82px "Times New Roman", serif';
      ctx.fillText('Jihan & Wahyu',w/2,h-165);
      ctx.font = '600 25px Arial, sans-serif';
      ctx.letterSpacing = '3px';
      ctx.fillText('23 OCTOBER 2026',w/2,h-112);
      ctx.font = '400 22px Arial, sans-serif';
      ctx.fillText('#JihanWahyu',w/2,h-70);
      ctx.shadowBlur = 0;
    }
    ...
    async function initialize(){
      try{
        const loaded = await Promise.all([
          ...
          loadImage(ASSETS.frame).catch(function(){ return null; }),
          loadImage(ASSETS.frameTop).catch(function(){ return null; })
        ]);
